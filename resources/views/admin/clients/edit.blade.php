...
@section('page-subtitle', 'قم بتعديل بيانات العميل ثم الحفظ')
...
<div class="form-card">
    ...
    <div class="form-body">
        <form method="POST" action="{{ route('admin.clients.update', $client->id) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label class="label" for="name">الاسم</label>
                <input id="name" name="name" type="text" class="input" value="{{ old('name', $client->name) }}" required>
                @error('name')<div class="error">{{ $message }}</div>@enderror
            </div>

            <div>
                <label class="label" for="email">البريد الإلكتروني</label>
                <input id="email" name="email" type="email" class="input" value="{{ old('email', $client->email) }}" required>
