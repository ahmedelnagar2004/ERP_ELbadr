...
@section('page-title', 'إنشاء مرتجع')
@section('page-subtitle', 'إضافة مرتجع فاتورة للنظام')
...
<div class="card shadow-lg rounded-lg p-4">
    <form action="{{ route('admin.returns.store') }}" method="POST" id="sale-form">
        @csrf
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                <label class="form-label fw-bold">الخزنة (التي سيتم الصرف منها)</label>
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                <label class="form-label fw-bold">المخزن (الذي سيتم الارجاع اليه)</label>
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                <label class="form-label fw-bold">طريقة الدفع (الارجاع)</label>
            ...
            <div class="col-md-6">
                <label class="form-label fw-bold">المبلغ المدفوع (المرتجع للعميل)</label>
                <div class="input-group">
                    <input type="number" name="paid_amount" id="paid_amount" class="form-control" value="0" min="0" step="0.01" readonly>
                    <span class="input-group-text">ج.م</span>
                </div>
                <small class="text-muted" id="remaining-amount-hint">المبلغ المتبقي (آجل): <span id="remaining-amount">0.00</span> ج.م</small>
            </div>
        </div>
        
        <!-- العناصر -->
        <div class="mb-3">
            <label class="form-label fw-bold">المنتجات المرتجعة</label>
            <table class="table table-bordered" id="items-table">
                <thead class="table-light">
                    <tr>
                        <th>المنتج</th>
                        <th>الكمية</th>
                        <th>السعر (سعر البيع الأصلي)</th>
                        <th>الإجمالي</th>
                        <th>
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                <label class="form-label fw-bold">نوع الخصم (الذي كان مطبقاً)</label>
            ...
            <div class="col-md-4 mb-3">
                <label class="form-label fw-bold">تكلفة الشحن (التي كانت مطبقة)</label>
        ...
        <div class="card mb-3">
            <div class="card-body">
                ...
                <div class="row mt-3">
                    <div class="col-12">
                        <h4 class="border-top pt-2">الصافي (المبلغ المستحق للعميل): <span id="net_total">0.00</span> ج.م</h4>
                        <div id="payment-summary" class="mt-2" style="display: none;">
                            <div class="d-flex justify-content-between">
                                <span>المبلغ المدفوع (نقداً/بنك):</span>
                                <span id="paid-amount-display">0.00 ج.م</span>
                            </div>
                            <div class="d-flex justify-content-between fw-bold">
                                <span>المبلغ المتبقي (يخصم من رصيد العميل):</span>
        ...
        <button type="submit" class="btn btn-danger">حفظ المرتجع</button>
    </form>
</div>
@endsection
...
$(document).ready(function() {
    ...
    $(document).on('change', '.item-select', function() {
        ...
        // Removed stock check for returns
